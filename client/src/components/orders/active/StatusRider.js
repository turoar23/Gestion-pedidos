const StatusRider = props => {
    ...
    const riderIsDelivering = orders.filter(order => order.status !== 'Completed' && order.status !== 'Active').length > 0;
	...
	const getLastUpdate = order => {
		const times = order.times.filter(
			time =>
				time.action !== 'fulfill_at' && time.action !== 'accepted_at'
		);
	...
	console.log(getLastUpdate(orders[0]));

	// Recuperamos el pedido más reciente actualizado
	const orderIdOld = console.log(orders);
	return (
		<div>
			<div>{rider}</div>
            <div></div>
