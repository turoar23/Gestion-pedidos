async function update() {
  ...
  if (orders.length > 0) {
    for (var i = 0; i < orders.length; i++) {
      ...
      clone.querySelector('#fulfill').textContent = `Entrega: ${fulfill
        .tz('Europe/Madrid')
        .format('LT')}`;
      ...
      clone.querySelector(
        '#zipCity'
      ).textContent = `${orders[i].address.zipcode} ${orders[i].address.city}`;
...
async function showOrdersList() {
  ...
  if (orders.length > 0) {
    for (var i = 0; i < orders.length; i++) {
      ...
      clone.querySelector('#from').textContent = `${orders[i].restaurant} - ${fulfill
        .tz('Europe/Madrid')
        .format('LT')}`;
      if (typeof orders[i].for_later !== 'undefined')
        clone.querySelector('#from').innerHTML += orders[i].for_later
