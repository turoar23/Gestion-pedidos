...
async function update() {
  ...
  if (orders.length > 0) {
    for (var i = 0; i < orders.length; i++) {
      ...
      if (durationExtra > 0) clone.querySelector('.duration-extra').style.color = 'red';
...
async function updateResumen() {
  ...
  if (orders.length > 0) {
    for (var i = 0; i < orders.length; i++) {
      ...
      clone.querySelector('.step1 .duration').textContent = differentTwoDate(
        accepted_at,
        fulfill_at
      );
      clone.querySelector('.step2 .time').textContent = formatTime(delivering);
      clone.querySelector('.step2 .duration').textContent = differentTwoDate(
        accepted_at,
        delivering
      );
      ...
      if (typeof orders[i].for_later !== 'undefined')
        clone.querySelector('.fulfill_at .time').innerHTML += orders[i].for_later
